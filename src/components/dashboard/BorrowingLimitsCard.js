    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium text-gray-700">โควต้าการยืม</span>
        ...
        <div className="mt-2 flex justify-between text-xs text-gray-500">
          <span>ยืมอยู่: {currentBorrowedCount} ชิ้น</span>
          <span>รอดำเนินการ: {pendingRequestsCount} ชิ้น</span>
          <span className={remainingQuota === 0 ? 'text-red-600 font-medium' : 'text-green-600 font-medium'}>
            ยืมได้อีก: {remainingQuota} ชิ้น
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-500">จำนวนสูงสุด</p>
              <p className="text-lg font-semibold text-gray-900">{maxItems} ชิ้น</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-500">ระยะเวลายืม</p>
              <p className="text-lg font-semibold text-gray-900">{maxDays} วัน</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-500">จองล่วงหน้า</p>
              <p className="text-lg font-semibold text-gray-900">{maxAdvanceBookingDays} วัน</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <p className="text-xs text-gray-500">
          {isEnabled ? (
            isDefault ? (
              <>ใช้ค่าเริ่มต้นสำหรับประเภท "{userTypeName}"</>
            ) : (
              <>สิทธิ์ตามประเภทผู้ใช้ "{userTypeName}"</>
            )
          ) : (
            <>ใช้ค่าเริ่มต้นของระบบ (ไม่ได้เปิดใช้งานการจำกัดตามประเภทผู้ใช้)</>
          )}
        </p>
      </div>
    </div>
