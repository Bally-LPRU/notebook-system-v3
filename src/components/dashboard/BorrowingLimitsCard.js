    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div className="mb-5">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium text-gray-600">โควต้าการยืม</span>
        ...
        <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
          <div
            className={`h-3 rounded-full transition-all duration-500 ${progressConfig.bar}`}
        ...
        <div className="mt-2 flex justify-between text-sm">
          <div className="flex gap-3">
            <span className="text-gray-500">
              <span className="font-medium text-blue-600">{currentBorrowedCount}</span> ยืมอยู่
            </span>
            <span className="text-gray-500">
              <span className="font-medium text-amber-600">{pendingRequestsCount}</span> รออนุมัติ
            </span>
          </div>
          <span className={`font-semibold ${progressConfig.text}`}>
            เหลือ {remainingQuota} ชิ้น
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 rounded-xl border border-blue-100">
          ...
          <span className="text-sm font-medium text-blue-700">สูงสุด {maxItems} ชิ้น</span>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50 rounded-xl border border-emerald-100">
          ...
          <span className="text-sm font-medium text-emerald-700">ยืมได้ {maxDays} วัน</span>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 bg-orange-50 rounded-xl border border-orange-100">
          ...
          <span className="text-sm font-medium text-orange-700">จองล่วงหน้า {maxAdvanceBookingDays} วัน</span>
        </div>
      </div>
    </div>
  );
