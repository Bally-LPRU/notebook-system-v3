...
/**
 * Generate confirmation phrase based on data types
 * Must match the phrase generated by DataManagementService._generateConfirmationPhrase
 */
...
const DeleteConfirmation = ({
  ...
  onConfirm,
  onCancel,
  onUpdateConfirmationPhrase
}) => {
  const [confirmationInput, setConfirmationInput] = useState('');

  // Generate the expected confirmation phrase based on selected data types
  const expectedPhrase = generateConfirmationPhrase(config.dataTypes);

  // Handle confirmation input change and update parent config
  const handleConfirmationChange = (e) => {
    const value = e.target.value;
    setConfirmationInput(value);
    // Update parent config with the confirmation phrase
    if (onUpdateConfirmationPhrase) {
      onUpdateConfirmationPhrase(value);
    }
  };
  ...
  const isConfirmationValid = confirmationInput.trim().toUpperCase() === expectedPhrase;
